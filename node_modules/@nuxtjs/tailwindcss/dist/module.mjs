...
import { resolvePath, tryResolveModule, useNuxt, useLogger, addTemplate, addTypeTemplate, isNuxtMajorVersion, addDevServerHandler, updateTemplates, findPath, resolveAlias, defineNuxtModule, getNuxtVersion, installModule, addImports, createResolver } from '@nuxt/kit';
...
const version = "6.13.3-20250420-215513-4eea740";
...
const createObjProxy = (configUpdatedHook, meta) => {
  const trackObjChanges = (configPath, path = []) => ({
    ...
    set(target, key, value) {
      ...
      if (JSONStringifyWithUnsupportedVals(target[key]) === JSONStringifyWithUnsupportedVals(value) || configUpdatedHook[configPath].endsWith(resultingCode)) {
...
const createInternalContext = async (moduleOptions, nuxt = useNuxt()) => {
  ...
  const resolveConfigs = (configs, nuxt2 = useNuxt()) => (Array.isArray(configs) ? configs : [configs]).filter((c) => Boolean(c) && c !== join(nuxt2.options.rootDir, "tailwind.config")).map(async (config, idx, arr) => {
    ...
    return configFile ? loadConfig({ configFile }).then(async (resolvedConfig) => {
      ...
      await nuxt2.callHook("tailwindcss:loadConfig", new Proxy(config2, trackObjChanges(resolvedConfigFile)), resolvedConfigFile, idx, arr);
